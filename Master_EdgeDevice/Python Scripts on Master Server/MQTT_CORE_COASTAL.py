...
SENSOR_DATA = {
    "waterVal": {"token": "WATERLEVEL1", "value": "default wtlv"},
    "hpa": {"token": "AIRPRESSURE1", "value": "default airp"}
    }
    ...
    with db_connection:
        with db_connection.cursor() as cursor:
            ...
            sql = """INSERT INTO weather_data_coastal(date, time, air_pressure, water_level) VALUES (
                        CURDATE(), CURTIME(), '%s', '%s');""" % (int(SENSOR_DATA["hpa"]["value"]), int(SENSOR_DATA["waterVal"]["value"]))
