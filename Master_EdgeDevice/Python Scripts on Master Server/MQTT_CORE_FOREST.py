# name: MQTT_CORE_FOREST.py
# auth: Mafaz Abrar Jan Chowdhury
# date: 16 MAY 2022
# desc: MQTT to FOREST DB - python script for Assignment 3
...
SENSOR_DATA = {
    "temperature": {"token": "TEMPERATURE2", "value": "default temp"},
    "humidity": {"token": "HUMIDITY2", "value": "default humd"},
    "gasValue": {"token": "SMOKE1", "value": "default smok"}
    }
...
def create_sql_table(db_connection):
    ...
    with db_connection:
        with db_connection.cursor() as cursor:
            ...
            sql = "SHOW TABLES LIKE 'weather_data_forest';" 
            cursor.execute(sql)
            ...
            if (result is None):
                sql = """CREATE TABLE weather_data_forest(
...
def record_data_in_db(db_connection):
    ...
    with db_connection:
        with db_connection.cursor() as cursor:
            ...
            sql = """INSERT INTO weather_data_forest(date, time, temperature, humidity, smoke) VALUES (
                        CURDATE(), CURTIME(), '%s', '%s', '%s');""" % (SENSOR_DATA["temperature"]["value"], SENSOR_DATA["humidity"]["value"], SENSOR_DATA["gasValue"]["value"])
...
def mqtt_on_connect(client, userdata, flags, rc):
    ...
    # on reconnect
    client.subscribe("/weather_data/forest")
    
    print("Subscribed to topic /weather_data/forest. ")
