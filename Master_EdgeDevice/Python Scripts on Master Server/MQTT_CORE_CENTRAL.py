# name: MQTT_CORE_CENTRAL.py
# auth: Mafaz Abrar Jan Chowdhury
# date: 16 MAY 2022
# desc: MQTT to CENTRAL DB - python script for Assignment 3
...
SENSOR_DATA = {
    "earthquake_duration": {"token": "EARTHQUAKE1", "value": "default eard"},
    "earthquake_severity": {"token": "EARTHQUAKE1", "value": "default ears"}
    }
...
def create_sql_table(db_connection):
    ...
    with db_connection:
        with db_connection.cursor() as cursor:
            ...
            sql = "SHOW TABLES LIKE 'weather_data_central';" 
            cursor.execute(sql)
            ...
            if (result is None):
                sql = """CREATE TABLE weather_data_central(
                            data_id INT NOT NULL AUTO_INCREMENT,
                            date DATE NOT NULL,
                            time TIME NOT NULL,
                            earthquake_duration FLOAT NOT NULL,
                            earthquake_severity FLOAT NOT NULL,
                            PRIMARY KEY(data_id)
                        );"""
...
def record_data_in_db(db_connection):
    ...
    with db_connection:
        with db_connection.cursor() as cursor:
            ...
            sql = """INSERT INTO weather_data_central(date, time, earthquake_duration, earthquake_severity) VALUES (
                        CURDATE(), CURTIME(), '%s', '%s');""" % (SENSOR_DATA["earthquake_duration"]["value"], SENSOR_DATA["earthquake_severity"]["value"])
...
def mqtt_on_connect(client, userdata, flags, rc):
    ...
    # on reconnect
    client.subscribe("/weather_data/central")
    
    print("Subscribed to topic /weather_data/central. ")
